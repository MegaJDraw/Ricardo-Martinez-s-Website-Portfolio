function GOL(canvas) {
	...
	this.rule_paths.push('glsl/390.frag');
...
GOL.prototype.start = function(canvas) {
    if (this.timer == null) {
        this.timer = setInterval(function(){
			...
			if(gol.stallframes) {gol.stall_count--;}
			if(gol.stall_count <= 0) {
				...
				for (var repeat_frame_i = 0; repeat_frame_i < gol.gpuframes; repeat_frame_i++) { 
					...
					if(gol.stallframes) {gol.stall_count = gol.stall_length;}
...
function addGUI() { 
    ...
    //gui.add(cont, 'bNext')			.name('NextRule').onFinishChange(nextrule);
    ...
    //gui.add(cont, 'bStall')			.name('Delay Render').onFinishChange(tog_stall);
    ...
    //gui.add(cont, 'nform', 0, 200)		.step(1.0).name('Nonuniform Seed').onFinishChange(set_nform);
    gui.add(cont, 'rule', 0, 500)			.step(1.0).name('Rulefile Num').onFinishChange(set_rule);
    //gui.add(cont, 'recframerate', 1, 1024)	.step(1.0).name('Record Frame-skip').onFinishChange(set_frame_capture);
    //gui.add(cont, 'stall_len', 0, 64)		.step(1.0).name('Render Frame Stall').onFinishChange(set_stall_len);
	...
	function nextrule(){
		...
		for (var i = 0; i < 10; i++) {
			gol.place_circle_pierce(gol.textures.layer0_0, gol.textures.rand0, gol.textures.layer0_1, Math.random()*gol.statesize[0], Math.random()*gol.statesize[1], Math.random()*128+1, 0, 9);
			gol.place_circle_pierce(gol.textures.layer0_0, gol.textures.rand0, gol.textures.layer0_1, Math.random()*gol.statesize[0], Math.random()*gol.statesize[1], Math.random()*128+1, 0, 8);
		} 
		document.getElementById("life").focus();
	}
	...
	function tog_stall(value){
		gol.stallframes = !gol.stallframes;
		gol.stall_count = 0;
		document.getElementById("life").focus();
	}
	...
	function set_frame_capture(value){
   	...
   	function set_stall_len(value){
		gol.toggle();
        gol.stall_length = Math.round(value);
        gol.stall_count = 0;
		gol.toggle();
		document.getElementById("life").focus();
	}
   	...
   	function set_nform(value){
		gol.toggle();
        gol.setRandom(gol.textures.rand0, gol.seedStrength, gol.seedMode);
		gol.toggle();
		/*for (var i = 0; i < value; i++) {
			gol.place_circle_pierce(gol.textures.layer0_0, gol.textures.rand0, gol.textures.layer0_1, Math.random()*gol.statesize[0], Math.random()*gol.statesize[1], Math.random()*256, 0, 6);
			gol.place_circle_pierce(gol.textures.layer0_0, gol.textures.rand0, gol.textures.layer0_1, Math.random()*gol.statesize[0], Math.random()*gol.statesize[1], Math.random()*256, 0, 7);
		} */
		for (var i = 0; i < value; i++) {
			gol.place_circle_pierce(gol.textures.layer0_0, gol.textures.rand0, gol.textures.layer0_1, Math.random()*gol.statesize[0], Math.random()*gol.statesize[1], Math.random()*128+1, 0, 9);
			gol.place_circle_pierce(gol.textures.layer0_0, gol.textures.rand0, gol.textures.layer0_1, Math.random()*gol.statesize[0], Math.random()*gol.statesize[1], Math.random()*128+1, 0, 8);
...
function myConfig() {
	...
	this.bStall = gol.stallframes;
	this.stall_len = gol.stall_length;
...
GOL.prototype.reset_rules = function(ruleval) {
	gol.rule_paths[0] = 'glsl/' + ruleval + '.frag';
	gol.programs = {
      
		Rule0:	gol.igloo.program('glsl/quad.vert', gol.rule_paths[0]),
		blurFilter:	gol.igloo.program('glsl/quad.vert', gol.rule_paths[1]),
	...
	console.log('ShaderLoaded ' + ruleval);
}
